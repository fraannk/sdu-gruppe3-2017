...
\begin{figure}[h]
\centering
\includegraphics[width=.90\textwidth]{billeder/fir_frekvenssampling.png}\label{fir_frekvenssampling}
...
Den grundlæggende tilgang til windowsmetoden er baseret på at det ideelle lavpas-, højpas-, båndpas-, og båndstop filter, har den følgende ikke-kausale impulsrespons på $-\infty < n < \infty$.

\begin{equation}
h_{d,LP}[n] = \frac{sin(\omega_c n)}{\pi n}
\end{equation}
\begin{equation}
h_{d,HP}[n] = \delta(n) - \frac{sin(\omega_c n)}{\pi n}
\end{equation}
\begin{equation}
h_{d,BP}[n] = \frac{sin(\omega_b n) - sin(\omega_a n)}{\pi n}
\end{equation}
\begin{equation}
h_{d,BS}[n] = \delta(n) - \frac{sin(\omega_b n) - sin(\omega_a n)}{\pi n}
\end{equation}
