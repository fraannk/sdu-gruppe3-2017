\section{Design af filter}\label{sec:design_filter}
	...
	Som filter til equalizeren bruges high-shelf, low-shelf og  peak/notch.
    Der blive kun anvendt 2. ordens filtre, da der ikke er brug for højere orden inde for audio. 
    ...
    I ligning \ref{eq:band} summeres båndstop- og båndpasfilteret for at kunne forstærke eller dæmpe signalet omkring center frekvensen.\\
   ...
   Hvor parametrene er $Q$, $G$, $f_0$. Heraf beregnes $w_0$ udfra ligning (),$\Delta \omega$ udfra ligning (), $\beta$ fra ligning (\ref{eq:beta}), $G_B$ beregnes fra ligning (\ref{eq:gain_b}).
   På figur (
       %\ref{fig:iir_peak}
